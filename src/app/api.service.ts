...
export class ApiService {
    ...
    postUser(user){
        this.http.post('http://localhost:63100/api/userquestion',user).subscribe(res =>{
            console.log(res)
        })
    }
    postTeacher(teacher){
        this.http.post('http://localhost:63100/api/teacher',teacher).subscribe (res =>{
            console.log(res)
        })
    }

    getAllTeachers(){
        return this.http.get('http://localhost:63100/api/teacher/all');
    }
